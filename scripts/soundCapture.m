function recordedaudio=soundCapture(pahandle, wavfilename, voicetrigger, maxsecs)
% Function to capture sound hacked together by Jim from the PsychPortAudio
% demo.  
%
%
...
%example: startTime = PsychPortAudio('Start', pahandle [, repetitions=1] [,
...
j = PsychPortAudio('GetStatus', pahandle);

% Stay in a little loop until keypress: 
while   ((length(recordedaudio) / j.SampleRate) < maxsecs)
    % Wait a second...
    WaitSecs(1);
